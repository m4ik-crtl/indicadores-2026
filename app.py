...
/* Número destacado */
.big-number { font-size: 2.4rem; font-weight: 700; color: #0094A4; font-family: 'DM Mono', monospace; }
.big-label { font-size: 0.78rem; text-transform: uppercase; letter-spacing: 0.08em; color: #7D8590; }
...
datas = st.sidebar.date_input("Período", [min_d, max_d], min_value=min_d, max_value=max_d)
...
if len(datas) == 2:
    ...
    def in_date(df, col='Data'):
        return (df[col] >= d0) & (df[col] <= d1)

    def tag_match(df):
        return df['Tags Tipo'].apply(lambda t: any(f in str(t) for f in filtro_tag))
    ...
    over_f, lin_f, lin_raw_f, blo_f, mai_f, lista_f = df_over, df_lin, df_lin_raw, df_blo, df_mai, df_lista
